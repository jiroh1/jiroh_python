...
#########추가
        ...
        btn_image = tkinter.Button(word_frame,padx=5, pady=5, text="순위권 내 가수", width=12, command=self.pop_up)
        btn_image.pack(side="right",padx=5, pady=5)
        btn_image2 = tkinter.Button(word_frame, padx=5, pady=5, text="가수별 좋아요 수", width=12, command=self.pop_up2)
...
#########추가
        ...
        # 검색버튼
        button_add_item = tkinter.Button(search_option, padx=5, pady=5, width = 12, text='검색', command = self.onButtonClick or self.onPressEnter )
        ...
        tree_add_item = tkinter.Button(tree_frame, padx=5, pady=5, width = 12, text='조회', command = self.treeview)
        tree_add_item.pack(side="right", padx=5, pady=5)
        tree_add_item = tkinter.Button(tree_frame, padx=5, pady=5, width=12, text='새로고침', command=self.treedelete)
        tree_add_item.pack(side="right", padx=5, pady=5)
        ...
        #실행
        btn_close = tkinter.Button(self, padx=5, pady=5, text="닫기", width=12, command=self.quit)
        btn_close.pack(side="right", padx=5, pady=5)
        btc_start = tkinter.Button(self, padx=5, pady=5, text="csv file 저장", width=12, command=self.save_text)
        btc_start.pack(side="right", padx=5, pady=5)
        btn_start = tkinter.Button(self, padx=5, pady=5, text="사진저장", width=12, command=self.save_pic)
...
########추가
    def pop_up(self):
        ...
        self.f = Frame(self.pop_up, height =500, width = 500,cursor = "hand2")
        self.f.pack()
